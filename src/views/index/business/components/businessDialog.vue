<template>
  <el-dialog :title="isAdd?'新增企业':'编辑企业'" :visible.sync="dialogFormVisible" center width="600px">
...
<script>
import { addBussiness, editoBussiness } from "@/api/bussiness.js";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    sureBtn() {
      this.$refs.formlist.validate(valid => {
        if (valid) {
          if (this.isAdd) {
            addBussiness(this.form).then(res => {
